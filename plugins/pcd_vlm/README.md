...
两条板端调优铁律:
1. **`npu_max_fps` 是省 CPU 的主旋钮**:事件型应用检测 10fps 足够,8 路 CPU 从 86%→43%。
2. **不要给 `frame_output` 指定 width/height**:跟随解码分辨率是零拷贝引用;一旦指定尺寸就走
   "IVPS 缩放+拷贝"路径,实测 8 路反而从 86% **涨到 97%**(反优化)。
