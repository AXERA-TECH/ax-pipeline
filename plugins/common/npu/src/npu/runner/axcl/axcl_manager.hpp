...
private:
    ...
    void run(int devid)
    {
        ...
        // IMPORTANT:
        // This worker runs in its own thread, so it must not assume any AXCL runtime state
        // inherited from the main thread. We rely on ax-video-sdk system init to call axclInit().
        // Here we only bind the device/runtime context for this thread.
        ...
        if (devid < 0 || devid >= static_cast<int>(lst.num)) {
            ALOGE("Invalid AXCL device index %d, total=%u.", devid, lst.num);
            initPromise.set_value(false);
            return;
        }
        const auto runtime_id = lst.devices[devid];
