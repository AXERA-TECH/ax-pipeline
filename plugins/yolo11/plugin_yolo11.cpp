...
// YOLO11 detection plugin.
//
// YOLO11's detection head is identical to YOLOv8-native (DFL, `4*reg_max` box
// channels + `num_classes` class channels concatenated per stride, output as
// NHWC by the AXERA "cut" export). This plugin is a foolproof preset: it fixes
// the head spec (strides {8,16,32}, reg_max 16) so a user only has to point it
// at a model (and set num_classes for custom-trained models). Preprocess
// defaults to letterbox (keep_aspect), matching how YOLO11 is exported.
...
int ax_plugin_init(const char* init_json, int32_t device_id, ax_plugin_handle_t* out_handle) {
    ...
    *out_handle = nullptr;
    ...
    // Foolproof YOLO11 head spec -- fixed, not user-tunable.
    axpipeline::npu::YoloDetOptions opt{};
    opt.strides = {8, 16, 32};
    opt.yolov8_reg_max = 16;
    ...
    if (!ctx->model.Init(opt, &err)) {
        std::fprintf(stderr, "[ax_plugin_yolo11] model init failed: %s\n", err.c_str());
        return -4;
    }
...
int ax_plugin_infer(ax_plugin_handle_t handle,
    ...
    if (!ctx->model.Infer(*frame, &dets, &err, tm_ptr)) {
        std::fprintf(stderr, "[ax_plugin_yolo11] Infer failed%s%s\n", err.empty() ? "" : ": ", err.c_str());
        return -3;
    }
    ...
    if (ctx->debug_timing) {
        ...
        if (now_us - ctx->last_timing_us >= ctx->timing_interval_us && ctx->timing_count > 0) {
            ...
            std::fprintf(stderr,
                         "[ax_plugin_yolo11] timing avg_us{pre=%llu infer=%llu post=%llu total=%llu} n=%llu\n",
