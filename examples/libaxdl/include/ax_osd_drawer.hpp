...
private:
    ...
    SimpleRingBuffer<cv::Mat> mRingBufferMatText, mRingBufferMatMask;
    int nWidth, nHeight;
    int index = -1;
...
public:
    ...
    {
        mRingBufferMatText.resize(num_rgn * AX_IVPS_REGION_MAX_DISP_NUM * SAMPLE_RINGBUFFER_CACHE_COUNT);
        mRingBufferMatMask.resize(num_rgn * AX_IVPS_REGION_MAX_DISP_NUM * SAMPLE_RINGBUFFER_CACHE_COUNT);
    ...
    void add_text(std::string text, axdl_point_t pos, ax_abgr_t color, float fontsize, int linewidth)
    {
        ...
        cv::Mat &canvas = mRingBufferMatText.next();
        canvas = cv::Mat(label_size.height + baseLine, label_size.width, CV_8UC4);
    ...
    void add_mask(axdl_bbox_t *box, axdl_mat_t *mask, ax_abgr_t color)
    {
        ...
        cv::Mat mask_target;
        cv::Mat &mask_color = mRingBufferMatMask.next();
        mask_color = cv::Mat(rect.height, rect.width, CV_8UC4);
